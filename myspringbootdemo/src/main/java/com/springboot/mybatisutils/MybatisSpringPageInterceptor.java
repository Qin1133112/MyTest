package com.springboot.mybatisutils;
...
public class MybatisSpringPageInterceptor implements Interceptor
{
	...
	private void queryTotal(MappedStatement mappedStatement, String replaceSql, Object parameterObject,
			BoundSql boundSql, BaseBean<?> model) throws Exception
	{
		StringBuffer countSql = new StringBuffer();

		if (model.getTotalMappedStatementId() != null && model.getTotalMappedStatementId().length() > 0)
		{
			MappedStatement totalMappedStatement = mappedStatement.getConfiguration()
					.getMappedStatement(model.getTotalMappedStatementId());
